...
router.put('/updateCartSize/:id', async (req, res) => {
    ...
    //     res.status(500).json({ message: 'Đã xảy ra lỗi' });
    await cartModel.findById(id, (err, cartItem) => {
        if (err) return res.status(500).send(err);
        if (!cartItem) return res.status(404).send('Sản phẩm không tìm thấy');

        cartItem.size = size;
        cartItem.save((err) => {
            if (err) return res.status(500).send(err);
            res.status(200).send('Cập nhật kích thước thành công');
        });
    });
    }
});
